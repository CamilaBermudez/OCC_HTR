...
YALTAI_TAGS = [
    ("BT000", "DamageZone"),
    ("BT001", "DigitizationArtefactZone"),
    ("BT002", "DropCapitalZone"),
    ("BT003", "GraphicZone"),
    ("BT004", "MainZone"),
    ("BT005", "MarginTextZone"),
    ("BT006", "MusicZone"),
    ("BT007", "NumberingZone"),
    ("BT008", "QuireMarksZone"),
    ("BT009", "RunningTitleZone"),
    ("BT010", "SealZone"),
    ("BT011", "StampZone"),
    ("BT012", "TableZone"),
    ("BT013", "TitlePageZone"),
    ...
    if tags_elem is not None:
        ...
        for tag_id, label in YALTAI_TAGS:
            ET.SubElement(
                tags_elem,
                "OtherTag",
                {"ID": tag_id, "LABEL": label, "DESCRIPTION": f"block type {label}"},
            )
    else:
...
if __name__ == "__main__":
    INPUT_FOLDER = os.path.join(
        PROJECT_ROOT, "data", "processed", "annotated_samples", "retrain", "annotations"
    )
    OUTPUT_FOLDER = os.path.join(
        PROJECT_ROOT, "data", "processed", "annotated_samples", "retrain", "yaltai_formatted"
    )
