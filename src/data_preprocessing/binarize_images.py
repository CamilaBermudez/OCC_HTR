...
def get_image_groups(input_path: Path, image_extensions):
    ...
    return {"root": [f for f in input_path.iterdir() if f.suffix.lower() in image_extensions]}


def binarize_image(
    input_img: str | Path, gaussian_filter: tuple[int, int] = (5, 5), method: str = "otsu_gaussian"
...
def binarize_and_save(
    ...
    gaussian_filter: tuple[int, int] = (5, 5),
    method: str = "otsu_gaussian",
    dry_run: bool = False,
) -> dict:
    ...
    stats = {
        "total_folders": len(image_groups),
        "total_images": 0,
        "success_count": 0,
        "failed_count": 0,
        "thresholds_summary": {},
    ...
    for group_name, image_files in tqdm(
        image_groups.items(), desc="Processing groups", unit="group"
    ):
        if not image_files:
            continue
        ...
        for img_path in tqdm(image_files, desc=f"Images in {group_name}", unit="file", leave=False):
            try:
                _, _, processed_name = format_filename(
                    base_name=img_path.stem, output_folder=folder_output_path
                )

                threshold, img_thr = binarize_image(
                    input_img=img_path, gaussian_filter=gaussian_filter, method=method
                )
        ...
        if not dry_run and thresholds:
            ...
            with open(summary_file, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"Thresholds: min={min(thresholds):.2f} / max={max(thresholds):.2f} / avg={np.mean(thresholds):.2f}\n"
                )
...
def run_binarization_pipeline(
    input_path: str | Path,
    output_base_dir: str | Path,
    logs_dir: str | None = None,
    run_name: str | None = None,
    gaussian_filter: tuple[int, int] = (3, 3),
    method: str = "otsu_gaussian",
    dry_run: bool = False,
) -> dict:
    ...
    logger.info(
        f"Starting binarization for {total_images} images across {len(image_groups)} folders..."
    )

    stats = binarize_and_save(
    ...
    success_rate = (
        stats["success_count"] / stats["total_images"] * 100 if stats["total_images"] > 0 else 0
    )

    logger.info(
        f"Binarization complete: {stats['success_count']}/{stats['total_images']} succeeded ({success_rate:.1f}%)"
    )
