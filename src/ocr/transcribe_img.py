...
def setup_simple_logging(
    logs_dir: str, task_name: str = "transcription", run_name: str | None = None
):
...
def fix_base_line(line_data, input_img_dir):
    ...
    line = BaselineLine(
        id="0", baseline=clipped_bl, boundary=[(0, 0), (w - 1, 0), (w - 1, h - 1), (0, h - 1)]
    )
    return line, im


def transcribe_image(
    img_inventory: str | Path,
    seg_path: str | Path,
    output_dir: str | Path,
    input_img_dir: str | Path,
    model,
    logs_dir: str | Path | None = None,
    task_name: str = "ocr_transcription",
    ...
    filt_inv = filt_inv[~filt_inv["was_removed"]]
    ...
    for folder_idx, img_ in enumerate(folders, 1):
        ...
        for idx, img_nm in zip(id_text_lines, text_lines, strict=False):
            global_idx += 1
            try:
                ...
                if idx < 0 or idx > len(json_data_lines):
                    logger.warning(
                        "Skipping %s: invalid json index (%d) for lines array of length %d",
                        img_nm,
                        idx,
                        len(json_data_lines),
                    )
                ...
                if global_idx % 20 == 0:
                    logger.info(
                        "Progress: %d lines processed | OK: %d | Failed: %d | Empty: %d",
                        global_idx,
                        total_processed,
                        total_failed,
                        total_empty,
