...
def main():
    ...
    output_folder = Path(args.output_folder) if args.output_folder else project_root / "data" / "processed" / "segmented_images"
    masks_folder = Path(args.masks_folder)

    logs_dir = Path(args.logs_dir) if args.logs_dir else project_root / "logs" / "segmentation"
    run_name = args.run_name or f"seg_{input_folder.name}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
