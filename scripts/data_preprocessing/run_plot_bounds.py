...
def main():
    ...
    input_dir = Path(args.input_dir) if args.input_dir else project_root / "data" / "raw" / "original_manuscript" / "reproduction14453_100"
    kraken_output_path = Path(args.kraken_output_path)
    output_dir = Path(args.output_dir) if args.output_dir else project_root / "data" / "processed" / "plotted_bounds"
    logs_dir = Path(args.logs_dir) if args.logs_dir else project_root / "logs" / "plotting"
    run_name = args.run_name or f"plot_{input_dir.name}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(
        f"\nDone: {result['processed']}/{result['total']} images | "
