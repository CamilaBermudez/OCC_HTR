...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="polygon",
        help="Crop method, polygon or rectangle",
    )
    ...
    output_folder = (
        ...
        else project_root / "data" / "processed" / "extracted_lines"
    )
    ...
    run_name = (
        args.run_name
        or f"crop_{input_folder.name}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )

    result = crop_all_images(
        input_folder=input_folder,
        output_kraken_path=output_kraken_path,
