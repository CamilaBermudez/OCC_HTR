...
def main():
    ...
    parser = argparse.ArgumentParser(description="Resize all filtered manuscript images")

    parser.add_argument("--input-folder", required=False)
    parser.add_argument("--output-folder", required=False)
    parser.add_argument(
        "--target-size", type=int, default=224, help="Target size for resized images (default: 224)"
    )
    ...
    output_folder = (
        ...
        else project_root / "data" / "processed" / "resized_samples"
    )
