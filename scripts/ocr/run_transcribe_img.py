...
def main():
    ...
    seg_path = (
        Path(args.seg_path)
        if args.seg_path
        else project_root / "data/processed/segmented_images/segmentation_20260505_172845"
    )
    input_img_dir = (
        Path(args.input_img_dir)
        if args.input_img_dir
        else project_root / "data/processed/filtered_images/20260510_160650/original/kept"
    )
    output_dir = (
        Path(args.output_dir) if args.output_dir else project_root / "data/processed/transcription"
    )
    img_inventory = (
        Path(args.img_inventory)
        if args.img_inventory
        else project_root / "data/processed/filtered_images/20260510_160650/filter_tracking.csv"
    )
    model_path = (
    ...
    run_name = (
        args.run_name or f"ocr_{input_img_dir.name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
